...
def process_audio_frame(frame: rtc.AudioFrame):
    ...
    print(
        f"Received audio frame: {frame.sample_rate} Hz, {frame.samples_per_channel} samples"
    )
...
async def process_audio(_audio_stream: rtc.AudioStream):
    async for event in _audio_stream:
        frame = event.frame
...
async def main(room: rtc.Room) -> None:
    ...
    def on_track_published(
        ...
        if publication.track.kind == rtc.TrackKind.KIND_AUDIO:
            _audio_stream = rtc.AudioStream(publication.track)
            asyncio.create_task(process_audio(_audio_stream))
    ...
    token = (
        api.AccessToken()
        .with_identity("python-bot-listen")
        .with_name("Python Bot Listen")
        .with_grants(
            api.VideoGrants(
                room_join=True,
                room="my-room",
...
def run_room():
    ...
    # Remove signal handling from the thread
    try:
        loop.run_forever()
    finally:
        loop.close()
